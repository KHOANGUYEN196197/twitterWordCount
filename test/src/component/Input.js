...
class Input extends Component {
  constructor(props) {
    ...
    this.recommendedMin = this.props.recommendedMin
      ? parseInt(this.props.recommendedMin, 10)
      : 0;
    this.recommendedMax = this.props.recommendedMax
      ? parseInt(this.props.recommendedMax, 10)
      : "infinite";
    ...
    if (
      this.checkForPixels([
        this.props.min,
        this.props.max,
        this.props.recommendedMin,
        this.props.recommendedMax,
      ])
    )
      this.type = "pixel";
  }
  ...
  results() {
    ...
    if (
      ...
      this.props.recommendedMax === undefined
    ) {
      var percentage =
        this.props.recommendedMin !== undefined
          ? this.current / this.recommendedMin
          : this.current / this.min;
    } else {
      var percentage =
        this.max !== "infinite"
          ? this.current / this.props.max
          : this.current / this.props.recommendedMax;
    }
    ...
    if (percentage === 0) {
      ...
      this.current < this.recommendedMin ||
      this.current > this.recommendedMax
    ) {
      this.setState({
        status: "warn",
        valid: false,
        ...
        <div className='bounded-input__limit-indicator'>
          ...
          <svg className='bounded-input__radial-counter' height='10' width='10'>
            ...
            <circle
              ...
              fill='none'
              strokeWidth='2'
              style={{ strokeDashoffset: progress, strokeDasharray: 38 }}
            ></circle>
          </svg>
        </div>
        {this.type === "pixel" ? (
          <span ref={(el) => (this.clone = el)} style={cloneStyles}>
            {value}
          </span>
        ) : (
          ""
